...
def import_complete_data():
    try:
        ...
        # 第一步：導入缺失的用戶
        print("👥 導入缺失的用戶...")
        users_match = re.search(r'INSERT INTO `users` VALUES\s*\((.*?)\);', content, re.DOTALL)
        if users_match:
                    ...
                    try:
                        ...
                        if not cursor.fetchone():
                            cursor.execute("""
                                ...
                                VALUES (%s, %s, %s, %s, %s)
                            """, (user_id, email, password, role, values[5].strip("'") if values[5] != 'NULL' else None))
                            print(f"✅ 導入用戶 {user_id}: {email}")
